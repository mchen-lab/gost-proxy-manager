...
export function LogViewer() {
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      ws.onmessage = (event) => {
        ...
        else if (data.type === "log") setLogs((prev) => {
  ...
  return (
    <Card className="h-full flex flex-col border-0 shadow-lg bg-slate-900 overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between py-3 px-5 border-b border-slate-800 bg-slate-950/50">
        <div className="flex items-center gap-3">
            ...
            <CardTitle className="text-sm font-mono text-slate-400 font-normal ml-2">backend/logs</CardTitle>
        </div>
        <div className="flex items-center gap-3">
            ...
            {logs.length === 0 ? (
              ...
              logs.map((log, idx) => (
                <div key={idx} className="flex gap-3 px-1 py-0.5 hover:bg-white/5 rounded-sm transition-colors cursor-default group">
                  ...
                  <span className={`shrink-0 font-bold w-12 text-center text-[10px] rounded px-1 self-start mt-0.5 ${
                    log.level === "ERROR" ? "bg-red-500/10 text-red-400" : 
                    log.level === "WARN" ? "bg-yellow-500/10 text-yellow-400" : 
                    log.level === "GOST" ? "bg-blue-500/10 text-blue-400" :
                    "bg-emerald-500/10 text-emerald-400" // APP/INFO logs in green
                  }`}>{log.level === "INFO" ? "APP" : log.level}</span>
